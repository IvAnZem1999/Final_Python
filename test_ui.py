"""Этот модуль содержит функции и проверки UI киносайта."""

from selenium.webdriver.common.by import By
import allure
from const import year_s, kind_s, country_s
import os
...
def test_search_year(search_main):
    ...
    with allure.step(
        "Проверка происхождения найденного контента " "в " + year_s + " год"
    ):
        year_find = search_main.wait_text(By.CLASS_NAME, "year", year_s)
...
def test_search_kind(search_main):
    ...
    with allure.step("Проверка, что найденное относится к жанру " + kind_s):
