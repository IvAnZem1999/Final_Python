"""Этот модуль содержит функции и проверки API киносайта."""

import requests
import allure
from const import url_k, long_name, country, period, diap, tip_f, drob
from class_api import ApiKino

api_kino = ApiKino()
response = requests.get(url_k, timeout=20)
...
def test_search_country():
    """Поиск видео по стране."""
    with allure.step("Формирование запроса"):
        cell_test = api_kino.get_video(
            "?", "&countries.name=", country)
    with allure.step("Найдено " + str(cell_test.json()["total"])
                     + " вхождений"):
        assert cell_test.json()["total"] > 1
...
def test_search_period():
    """Поиск видео по временному периоду."""
    with allure.step("Формирование запроса"):
